async function cacheApproxGeo() {
  try {
    if (localStorage.getItem("loveGeoApprox")) return;
    ...
    localStorage.setItem("loveGeoApprox", JSON.stringify(geo));
  } catch (err) {
    localStorage.setItem("loveGeoApprox", JSON.stringify({}));
  }
}
